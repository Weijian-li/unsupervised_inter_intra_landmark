...
def main():
    ...
    assert os.path.exists('data_{}.pkl'.format(folder)), 'Data not extracted yet'
    ...
    print(np.mean(all_errors[e][0], axis=1))
...
def NMSE(landmarks_gt, landmarks_regressed):
    ...
    # occular_distances = np.linalg.norm(eyes[0, 0, :] - eyes[0, 1, :])
...
def train_regressor(X,Y,l,size=64):
    center = size/2
    Xtmp = X/center - 0.5  # Normalize to [-0.5, 0.5]
    X0 = Xtmp.mean(axis=0, keepdims=True)
    Xtmp = Xtmp - np.ones((Xtmp.shape[0], 1)) @ X0.reshape(1, -1)  # Subtract Mean
